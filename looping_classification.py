...
train_file_paths = ['train-train.csv']
test_file_paths = ['train-test.csv']

log_file = "logs/classification_results.log"
...
for train_path, test_path in zip(train_file_paths, test_file_paths):
    ...
    for stage1_name, stage1_model in models.items():
        ...
        if stage1_name == 'Decision Tree (Gini)':
            log_message(f"Tree Depth: {stage1_model.get_depth()}")
        elif stage1_name == 'Random Forest (Gini)':
            ...
            log_message(f"Random Forest Depths -> Avg: {avg_depth:.2f}, Max: {max_depth}, Min: {min_depth}")
